...
nologging;
create unique index CMPI_PP09_UI on CMPI_PP09 (FID, BOORU)
  nologging;
...
prompt
prompt Creating table CMPI_XX11
prompt ========================
prompt
create table CMPI_XX11
(
  booru VARCHAR2(30),
  fid   NUMBER,
  cnt   NUMBER,
...
nologging;
create unique index CMPI_XX11_UI on CMPI_XX11 (FID, BOORU)
  nologging;
...
prompt
prompt Creating table CMPO_XX11
prompt ========================
prompt
create table CMPO_XX11
(
  booru    VARCHAR2(30),
  fid      NUMBER,
  obj      NUMBER,
...
nologging;
create index CMPO_XX11_IN on CMPO_XX11 (FID, BOORU)
  nologging;
...
prompt
prompt Creating table DET_BC11
prompt =======================
prompt
create table DET_BC11
(
  batch_id VARCHAR2(8) not null,
  booru    VARCHAR2(30) not null,
  fid      NUMBER not null,
...
nologging;
create unique index DET_BC11_UI on DET_BC11 (FID, BOORU, OID, BATCH_ID)
  nologging;

prompt
prompt Creating table DET_BCX
prompt ======================
prompt
create table DET_BCX
(
  batch_id VARCHAR2(8) not null,
  booru    VARCHAR2(30) not null,
  fid      NUMBER not null,
...
nologging;
create unique index DET_BCX_UI on DET_BCX (FID, BOORU, OID, BATCH_ID)
  nologging;
...
prompt
prompt Creating table DET_SM03
prompt =======================
prompt
create table DET_SM03
(
  batch_id VARCHAR2(8) not null,
  booru    VARCHAR2(30) not null,
  fid      NUMBER not null,
...
nologging;
create unique index DET_AS03_UI on DET_SM03 (FID, BOORU, OID)
  nologging;
...
prompt
prompt Creating table DET_XX11
prompt =======================
prompt
create table DET_XX11
(
  batch_id VARCHAR2(8) not null,
  booru    VARCHAR2(30) not null,
  fid      NUMBER not null,
...
nologging;
create unique index DET_XX11_UI on DET_XX11 (FID, BOORU, OID)
  nologging;

prompt
prompt Creating table DET__W
prompt =====================
prompt
create table DET__W
(
...
nologging;
create unique index DET_W_UI on DET__W (FID, BOORU, OID)
  nologging;
...
prompt
prompt Creating table DIR_SM03
prompt =======================
prompt
create table DIR_SM03
(
  booru VARCHAR2(300),
  fid   NUMBER,
  fline VARCHAR2(4000)
...
nologging;
create index DIR_AS03_I on DIR_SM03 (FID, BOORU)
  nologging;

prompt
prompt Creating table DIR_XX11
prompt =======================
prompt
create table DIR_XX11
(
  booru VARCHAR2(300),
  fid   NUMBER,
  fline VARCHAR2(4000)
...
nologging;
create index DIR_XX11_I on DIR_XX11 (FID, BOORU)
  nologging;
...
prompt
prompt Creating table EXIF_SM03
prompt ========================
prompt
create table EXIF_SM03
(
  booru    VARCHAR2(300),
  fid      NUMBER,
  iw       VARCHAR2(48),
...
prompt
prompt Creating table EXIF_XX11
prompt ========================
prompt
create table EXIF_XX11
(
  booru    VARCHAR2(300),
  fid      NUMBER,
  iw       VARCHAR2(48),
...
prompt
prompt Creating table STR_BC11
prompt =======================
prompt
create table STR_BC11
(
  booru VARCHAR2(30) not null,
  fid   NUMBER not null,
  h     NUMBER,
...
nologging;
create unique index STR_BC11_UI on STR_BC11 (FID, BOORU)
  nologging;

prompt
prompt Creating table STR_BC11S
prompt ========================
prompt
create table STR_BC11S
(
  booru VARCHAR2(30) not null,
  fid   NUMBER not null,
  h     NUMBER,
...
nologging;
create unique index STR_BC11S_UI on STR_BC11S (FID, BOORU)
  nologging;

prompt
prompt Creating table STR_BCX
prompt ======================
prompt
create table STR_BCX
(
  booru VARCHAR2(30) not null,
  fid   NUMBER not null,
  ar    NUMBER,
...
nologging;
create unique index STR_BCX_UI on STR_BCX (FID, BOORU)
  nologging;

prompt
prompt Creating table STR_XX10
prompt =======================
prompt
create table STR_XX10
(
  booru VARCHAR2(30) not null,
  fid   NUMBER not null,
  h     NUMBER,
...
nologging;
create unique index STR_XX10_UI on STR_XX10 (FID, BOORU)
  nologging;

prompt
prompt Creating table STR_XX11
prompt =======================
prompt
create table STR_XX11
(
  booru VARCHAR2(30) not null,
  fid   NUMBER not null,
  h     NUMBER,
...
nologging;
create unique index STR_XX11_UI on STR_XX11 (FID, BOORU)
  nologging;
...
prompt
prompt Creating table TR_SM03
prompt ======================
prompt
create table TR_SM03
(
  batch_id VARCHAR2(4) not null,
  obj      NUMBER not null,
  x        NUMBER not null,
...
nologging;
create unique index TR_AS03_UI on TR_SM03 (FID, BOORU, OID)
  nologging;
...
prompt
prompt Creating table TR_XX11
prompt ======================
prompt
create table TR_XX11
(
  batch_id VARCHAR2(4) not null,
  obj      NUMBER not null,
  x        NUMBER not null,
...
nologging;
create unique index TR_XX11_UI on TR_XX11 (FID, BOORU, OID)
  nologging;
...
select f.booru, f.fid, f.oid, b.oid boid, 'U' lvl, f.exif_ar,
       ...
       round(2*abs(nvl(b.x,f.x)-f.x)/(f.w+nvl(b.w,f.w)),2) tilt, -- dX to Width  tilt, vertical=0
       round(2*(nvl(b.y,f.y)-f.y)/(f.h+nvl(b.h,f.h)),2) tilty    -- dY to Height tilt, upside-down<0
       , f.batch_id fbat, b.batch_id bbat, f.fname fline
  from detbc f
  left join detbc b on f.booru=b.booru and f.fid=b.fid and b.oup=f.oid -- UPPER connection (head to something)
 where f.oid like 'H%' and f.odn is not null and f.exif_ar is not null
union
select b.booru, b.fid, b.oid, p.oid boid, 'D' lvl, b.exif_ar,
       ...
       objp.nn(substr(b.batch_id,1,2)||b.obj) fname, round(b.prob,2) prob,  round((b.x-b.w/2)*b.exif_iw) x, round((b.y-b.h/2)*b.exif_ih) y, round(b.w*b.exif_iw) w, round(b.h*b.exif_ih) h,
       objp.nn(substr(p.batch_id,1,2)||p.obj) bname, round(p.prob,2) oprob, round((p.x-p.w/2)*p.exif_iw) ox, round((p.y-p.h/2)*p.exif_ih) oy, round(p.w*p.exif_iw) ow, round(p.h*p.exif_ih) oh,
       round(2*abs(nvl(p.x,b.x)-b.x)/(b.w+nvl(p.w,b.w)),2) tilt, -- dX to Width  tilt, vertical=0
       round(2*(nvl(p.y,b.y)-b.y)/(b.h+nvl(p.h,b.h)),2) tilty    -- dY to Height tilt, upside-down<0
       , b.batch_id fbat, b.batch_id bbat, b.fname fline
  from detbc b
  left join detbc p on p.booru=b.booru and p.fid=b.fid and p.oup=b.oid -- DOWN connection
 where b.oup is not null and b.odn is not null and b.exif_ar is not null
/*union
select b.booru, b.fid, b.oid, p.oid boid, 'P' lvl, b.exif_ar,
       sum(case when b.oid like 'B%' then 1 else 0 end) over (partition by b.booru, b.fid) nn,
  ...
  from detbc b
  left join detbc p on p.booru=b.booru and p.fid=b.fid and p.oxx=b.oid -- PPXX connection
 where p.oxx is not null and b.exif_ar is not null*/
union -- NOT CONNECTED
select f.booru, f.fid, f.oid, 'none' boid, 'N' lvl, f.exif_ar,
       ...
       objp.nn(substr(f.batch_id,1,2)||f.obj) fname, round(f.prob,2) prob, round((f.x-f.w/2)*f.exif_iw) x, round((f.y-f.h/2)*f.exif_ih) y, round(f.w*f.exif_iw) w, round(f.h*f.exif_ih) h,
       case when nvl(f.suppr,-1)<0 then 'none' else 'suppr' end bname, 0 oprob, 0 ox, 0 oy, 0 ow, 0 oh, 0 tilt, 0 tilty
       , f.batch_id fbat, f.batch_id bbat, f.fname fline
 from detbc f
where f.oup is null and f.odn is null /*and f.oxx is null*/ and f.exif_ar is not null -- and nvl(f.suppr,-1)<0 -- and f.prob>=0.55
;
...
select t0.booru, t0.fid, t0.oid, objp.nn('XX'||t0.obj) obj0,
       round(2*abs(nvl(t2.x,t1.x)-t0.x)/(t0.w+nvl(t2.w,t1.w)),2) tilt, -- dX to Width  tilt, vertical=0
       round(2*(nvl(t2.y,t1.y)-t0.y)/(t0.h+nvl(t2.h,t1.h)),2) tilty,   -- dY to Height tilt, upside-down<0
       ...
       count(*) over (partition by t0.booru, t0.fid) nn, t0.exif_ar
  from detbc t0
...
select f.booru, f.fid, f.oid, 'none' boid, 'N' lvl, f.exif_ar,
       sum(case when f.oid like 'H%' then 100 when f.oid like 'B%' then 10 else 1 end) over (partition by f.booru, f.fid) nn,
       objp.nn(substr(f.batch_id,1,2)||f.obj) fname, round(f.prob,2) prob, round((f.x-f.w/2)*f.exif_iw) x, round((f.y-f.h/2)*f.exif_ih) y, round(f.w*f.exif_iw) w, round(f.h*f.exif_ih) h,
       'none' bname, 0 oprob, 0 ox, 0 oy, 0 ow, 0 oh, 0 tilt, 0 tilty
 from det f
...
-- ZERO pass
  merge into det o using (
    select booru, fid, boid, max(suid) suid, max(suppr) suppr from (
      select a.booru, a.fid, a.oid, b.oid boid, 90000+100*b.obj+a.obj suppr,
             first_value(a.oid) over (partition by a.booru, a.fid, b.oid order by iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h) desc ) suid
       from det a
       join det b on a.booru=b.booru and a.fid=b.fid and a.oid!=b.oid and (a.batch_id=b.batch_id or crs!=0)
                   and (a.prob>b.prob or (a.prob=b.prob and a.oid>b.oid))
                   and ( iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h)>0.9 and
                         iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h,1)>0.95 and
                         iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h,2)>0.95 )
  ...
  where o.suppr is null and o.fname like pth ;
  dbms_output.put_line('9I='||SQL%ROWCOUNT);
  commit;
  merge into det o using (
    select booru, fid, boid, max(suid) suid, max(suppr) suppr from (
      select a.booru, a.fid, a.oid, b.oid boid, 80000+100*b.obj+a.obj suppr,
             first_value(a.oid) over (partition by a.booru, a.fid, b.oid order by iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h) desc ) suid
       from det a
       join det b on a.booru=b.booru and a.fid=b.fid and a.oid!=b.oid and (a.batch_id=b.batch_id or crs!=0)
                   and (a.prob>b.prob or (a.prob=b.prob and a.oid>b.oid))
  ...
  when matched then update set o.sid=n.suid, o.suppr=n.suppr
  where o.suppr is null and o.fname like pth ;
  dbms_output.put_line('8I='||SQL%ROWCOUNT);
  commit;
  -- for CROSSBATCHES go deeper
  if crs!=0 then
    merge into det o using (
      select booru, fid, boid, max(suid) suid, max(suppr) suppr from (
        select a.booru, a.fid, a.oid, b.oid boid, 70000+100*b.obj+a.obj suppr,
               first_value(a.oid) over (partition by a.booru, a.fid, b.oid order by iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h) desc ) suid
         from det a
         join det b on a.booru=b.booru and a.fid=b.fid and a.oid!=b.oid and substr(a.oid,1,1)=substr(b.oid,1,1)
                     and (a.prob>b.prob or (a.prob=b.prob and a.oid>b.oid))
                     and ( iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h)>0.7 and
                           iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h,1)>0.8 and
                           iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h,2)>0.8 )
                     and b.suppr is null and b.fname like pth
        where a.suppr is null and a.fname like pth
        ) group by booru, fid, boid
      ) n on (n.fid=o.fid and n.booru=o.booru and o.oid=n.boid)
    when matched then update set o.sid=n.suid, o.suppr=n.suppr
    where o.suppr is null and o.fname like pth ;
    dbms_output.put_line('7C='||SQL%ROWCOUNT);
    commit;
    merge into det o using (
      select booru, fid, boid, max(suid) suid, max(suppr) suppr from (
        select a.booru, a.fid, a.oid, b.oid boid, 60000+100*b.obj+a.obj suppr,
               first_value(a.oid) over (partition by a.booru, a.fid, b.oid order by iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h) desc ) suid
         from det a
         join det b on a.booru=b.booru and a.fid=b.fid and a.oid!=b.oid and substr(a.oid,1,1)=substr(b.oid,1,1)
                     and (a.prob>b.prob or (a.prob=b.prob and a.oid>b.oid))
                     and ( iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h)>0.6 and
    ...
    when matched then update set o.sid=n.suid, o.suppr=n.suppr
    where o.suppr is null and o.fname like pth ;
    dbms_output.put_line('6C='||SQL%ROWCOUNT);
    commit;
  end if;  
end nmsdet;
/
    ...
    select booru, fid, oid, max(boid) boid from (
        select a.booru, a.fid, a.oid,
         ...
         join det b on a.booru=b.booru and a.fid=b.fid
                      ...
                      and (iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h, 1)>0.5 or
                           iou (a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h, 2)>0.5)
...
prompt
prompt Creating trigger DET_AS03_BR
prompt ============================
prompt
CREATE OR REPLACE TRIGGER 
DET_as03_BR before insert on "DET_SM03" for each row
declare
...
prompt
prompt Creating trigger DET_W_BR
prompt =========================
prompt
CREATE OR REPLACE TRIGGER 
DET_w_BR before insert on "DET__W" for each row
declare
  -- local variables here
begin
  if :new.booru is null then -- undefined yet
...
prompt
prompt Creating trigger DET_XX11_BR
prompt ============================
prompt
CREATE OR REPLACE TRIGGER DET_xx11_BR before insert on DET_xx11 for each row
declare
  -- local variables here
begin
  if :new.booru is null then -- undefined yet
...
prompt
prompt Creating trigger TR_AS03_BR
prompt ===========================
prompt
CREATE OR REPLACE TRIGGER 
TR_as03_BR before insert on "TR_SM03" for each row
declare
  -- local variables here
begin
...
prompt
prompt Creating trigger TR_XX11_BR
prompt ===========================
prompt
CREATE OR REPLACE TRIGGER TR_xx11_BR before insert on TR_xx11 for each row
declare
  -- local variables here
begin
  if :new.booru is null then
