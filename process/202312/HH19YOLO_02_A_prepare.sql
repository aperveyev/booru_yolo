----- ************************************************************ RESULTS SET **************************************
create or replace synonym det for det_xx10 -- DETECTIONS
...
----- ************************************************************ TRAIN SET **************************************
create or replace synonym tr   for tr_xx10   -- TRAINING DATASET
create or replace synonym dir  for dir_xx10
create or replace synonym exif for exif_xx10

-- patch filenames (to it only afred ddaset extension)
create table dir_xx10 as -- drop table dir_xx10
select booru, fid, fline
from dir_bs_ext_v 
where fid is not null

-- create index dir_xx10_i on dir_xx10 (fid,booru) -- 
-- select * from dir_xx10 where (fid,booru) in (select fid, booru from dir_xx10 group by fid, booru having count(*)>2)

create table exif_xx10 as -- drop table exif_xx10
...
update tr y set iname=(select min(fline) from dir b where b.fid=y.fid and b.booru=y.booru and fline like '%.jpg') where iname is null
update tr y set lname=(select min(fline) from dir b where b.fid=y.fid and b.booru=y.booru and fline like '%.txt') where lname is null
